...
func (db *DB) setPin(batch *leveldb.Batch, item shed.Item, pinIncrement uint64) (gcSizeChange int64, err error) {
	...
	db.logger.Debug("pinTrace:setPin: pinned", "chunk", swarm.NewAddress(item.Address).String(), "increment", pinIncrement, "pinCount", item.PinCounter)
...
func (db *DB) setUnpin(batch *leveldb.Batch, addr swarm.Address, pinIncrement uint64) (gcSizeChange int64, err error) {
	...
	if item.PinCounter > pinIncrement {
		item.PinCounter -= pinIncrement
		db.logger.Debug("pinTrace:setUnpin: decrement", "chunk", swarm.NewAddress(item.Address).String(), "decrement", -pinIncrement, "pinCount", item.PinCounter)
		return 0, db.pinIndex.PutInBatch(batch, item)
	}

	// PinCounter == 0

	db.logger.Debug("pinTrace:setUnpin: decrement Deleted!", "chunk", swarm.NewAddress(item.Address).String(), "pinCount", item.PinCounter, "decrement", pinIncrement)
	...
	if err != nil {
		if !errors.Is(err, leveldb.ErrNotFound) {
			db.logger.Debug("pinTrace:setUnpin: retrievalAccessIndex.Get err", "chunk", swarm.NewAddress(item.Address).String(), "err", err)
			return 0, fmt.Errorf("get retrieval access index: %w", err)
		}
		item.AccessTimestamp = now()
		err = db.retrievalAccessIndex.PutInBatch(batch, item)
		if err != nil {
			db.logger.Debug("pinTrace:setUnpin: retrievalAccessIndex.Put err", "chunk", swarm.NewAddress(item.Address).String(), "err", err)
			return 0, err
		}
	} else {
		item.AccessTimestamp = i.AccessTimestamp
